...
void WiFiManager::handleRoot() {
  ...
  page += "<h1>";
  page += F("WiFi Manager");//_apName;
  page += "</h1>";
  page += F("<h3>ตั้งค่า WiFi เพื่อเชื่อมต่ออินเทอร์เน็ต</h3>");
  page += FPSTR(ESP32_HTTP_PORTAL_OPTIONS);
...
void WiFiManager::handleWifi(boolean scan) {
    ...
    if (n == 0) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (_minimumQuality == -1 || _minimumQuality < quality) {
          String item = FPSTR(ESP32_HTTP_ITEM);
          String rssiQ;
          rssiQ += quality;
  ...
  page += FPSTR(ESP32_HTTP_FORM_START);
  char parLength[2];
  ...
  for (int i = 0; i < _paramsCount; i++) {
    ...
    String pitem = FPSTR(ESP32_HTTP_FORM_PARAM);
  ...
  if (_sta_static_ip) {
    ...
    String item = FPSTR(ESP32_HTTP_FORM_PARAM);
    item.replace("{i}", "ip");
    item.replace("{n}", "ip");
    ...
    item = FPSTR(ESP32_HTTP_FORM_PARAM);
    item.replace("{i}", "gw");
    item.replace("{n}", "gw");
  ...
  page += FPSTR(ESP32_HTTP_FORM_END);
  page += FPSTR(ESP32_HTTP_SCAN_LINK);
